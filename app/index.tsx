...
export default function TaskLanding() {
  ...
  useEffect(() => {
    (async () => {
      ...
      else setTasks(["Task 1", "Task 2", "Task 3"]); // default tasks
    })();
  }, []);
  ...
  const saveTask = (newValue: string) => {
    if (editIndex !== null) {
      const updated = [...tasks];
  ...
  const addTaskTop = () => setTasks(["New Task " + (tasks.length + 1), ...tasks]);

  const addTaskBottom = () => setTasks([...tasks, "New Task " + (tasks.length + 1)]);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.taskRow}>
        <TouchableOpacity style={styles.button} onPress={addTaskTop}>
          <Text>[New Task (add to top)]</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.button} onPress={addTaskBottom}>
